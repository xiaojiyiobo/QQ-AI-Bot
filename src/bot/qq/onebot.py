...
class QQOneBot:
    ...
    async def connect(self):
        async with websockets.connect(
            self.ws_url, open_timeout=10, close_timeout=5, ping_interval=20, ping_timeout=20
        ) as ws:
            yield ws
    ...
    async def request(self, action: str, params: dict | None = None, timeout: float = 10) -> dict:
        echo = f"qq-ai-bot-request-{uuid.uuid4().hex}"
    ...
    async def send_text(self, user_id: int, message: str) -> None:
        ...
        async with websockets.connect(self.ws_url, open_timeout=10, close_timeout=5) as ws:
            await ws.send(json.dumps({
                "action": "send_msg",
                "params": {"message_type": "private", "user_id": user_id, "message": message},
                "echo": echo,
            }, ensure_ascii=False))
            async for raw in ws:
                result = json.loads(raw)
    ...
    async def get_image(self, file_id: str) -> tuple[bytes, str]:
        ...
        async with websockets.connect(self.ws_url, open_timeout=10, close_timeout=5) as ws:
            await ws.send(json.dumps({"action": "get_image", "params": {"file": file_id}, "echo": echo}, ensure_ascii=False))
            async for raw in ws:
                result = json.loads(raw)
                if result.get("echo") != echo:
                    continue
                ...
                if path and os.path.isfile(path):
                    with open(path, "rb") as f:
                        image_data = f.read()
                    return image_data, guessed_mime or data.get("type") or "image/jpeg"
                if url:
                    with urllib.request.urlopen(url, timeout=30) as response:
                        image_data = response.read()
                        response_mime = response.headers.get("Content-Type", "image/jpeg").split(";")[0].strip()
                    return image_data, guessed_mime or response_mime or "image/jpeg"
                raise RuntimeError(f"NapCat get_image returned no usable path/url: {data}")
